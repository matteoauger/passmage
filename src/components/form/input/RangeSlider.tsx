...
export function RangeSlider({ label, value, onChange, min, max, className }: Props) {
            ...
            <div className='flex justify-between items-center'>
                ...
                <div className='flex items-center gap-2'>
                    <input
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                                const newValue = Math.min(Math.max(manualValue || min, min), max)
                                onChange(newValue)
                            }
                        }}
                        className='w-14 text-center bg-white border border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500'
                    />
                </div>
            </div>
